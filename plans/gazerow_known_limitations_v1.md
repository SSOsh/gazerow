...
| 7 | Finder sidebar row는 candidate coverage 보강 후 fixed task 재평가가 필요하다 | TICKET-010 실제 click task에서 sidebar item task fail 후 `AXRow` / `AXCell` / `AXImage` 수집 보강 |
| 8 | VS Code Activity Bar item은 candidate coverage 보강 후 fixed task 재평가가 필요하다 | TICKET-010 실제 click task에서 Activity Bar 이동 task fail 후 `AXRow` / `AXCell` / `AXImage` 수집 보강 |
...
| 앱 | 등급 |
| --- | --- |
| Finder | Limited: sidebar row fixed task 재평가 필요 |
| Safari | Evaluation pass |
| Chrome | Evaluation pass |
| VS Code | Limited: Activity Bar fixed task 재평가 필요 |
