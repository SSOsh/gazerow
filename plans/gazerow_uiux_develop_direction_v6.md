# keyCursor UI/UX Develop Direction v6
...
| v6 확정 | 2026-07-13 | `keyCursor`, 최초 튜토리얼, 한글 변환 무표시, `/`·`;` 유지, 창 후보 6개, dark HUD 확정 |
...
| --- | --- | --- | --- |
| PD-01 | 사용자 표시 이름 | `keyCursor` | v6에서 고정 |
...
제품명 대소문자는 `keyCursor`가 정확한 표기다. 문장 시작, 메뉴바, Settings, README 제목에서도 임의로 `KeyCursor`로 바꾸지 않는다.
...
8. 사용자 노출 이름은 `keyCursor`이며 내부 호환 식별자는 `GazeRow`를 유지한다.
...
- [ ] `keyCursor` 신규 앱 아이콘 제작.
- [ ] 튜토리얼 애니메이션 고도화.

### 4.3 제외 범위

- Swift Package, executable target, module 이름 `GazeRow` 변경.
...
| 이름 | `Sources/GazeRow/Infrastructure/AppState.swift` 등 | `GazeRow` 노출 | 사용자 표시 이름 `keyCursor` |
...
- `Package.swift` package/target `GazeRow`.
- executable 내부 이름이 script/signing 호환성에 필요하면 `GazeRow` 유지.
- source/test 디렉터리.
...
- `.app` 디렉터리만 `keyCursor.app`으로 바꾸는 경우 내부 executable lookup을 검증한다.
- 기존 `GazeRow.app`과 새 `keyCursor.app`이 동시에 남는 update 상황을 배포 문서에 기록한다.
...
### T-08 `keyCursor` 표시 이름 적용

- [x] `AppState.appName`을 `keyCursor`로 변경한다.
- [x] 사용자 노출 `GazeRow` literal을 `keyCursor` 기준으로 정리한다.
...
검색 결과의 모든 `GazeRow`는 package/module/path/internal compatibility 목적이어야 한다. 사용자 노출 `KeyCursor` 오표기도 남지 않아야 한다.
...
- [ ] `AppState.appName == "keyCursor"`다.
- [ ] 한국어와 영어 tutorial 문구가 모두 존재한다.
- [ ] 사용자 문구에 `KeyCursor` 오표기가 없다.
...
- [ ] 사용자 표시 이름은 정확히 `keyCursor`다.
- [ ] package/module/defaults key는 기존 `GazeRow` 호환성을 유지한다.
