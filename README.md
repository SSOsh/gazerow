...
> overlay activation smoke와 5개 앱 실제 click task를 수행했다. 결과는
> Safari/Chrome/System Settings pass, Finder/VS Code fail이며, 30분
> crash-free session도 통과했다. 이후 Finder/VS Code 재평가를 위해
> launch-option click result stdout 출력과 overlay panel 좌표 변환을 추가했고,
> Finder overlay 표시 smoke를 통과했다. 내부 사용자 3명 gate는 ED-008에
> 따라 Post-MVP로 defer했다. 현재 차단 항목은 Finder/VS Code fixed task
> 재평가와 최종 go/no-go 판정이다.
...
| TICKET-010 | In progress | runtime wiring, 권한 요청 동선, Accessibility 권한 부여, 5개 앱 overlay activation smoke, 실제 click task, 30분 crash-free session, click result stdout reporter, overlay 좌표 보정 완료. 내부 사용자 gate는 Post-MVP defer. Finder/VS Code 재평가는 미완료 |
| TICKET-011 | Draft prepared / blocked | freeze package, default audit, verification script, distribution checklist 준비 완료. 최종 확정은 Finder/VS Code 재평가와 go/no-go 판정 필요 |
...
- [x] overlay panel AX/AppKit 좌표 변환 및 Finder 표시 smoke
...
- **TICKET-010**: Finder / VS Code fixed task 재평가와 최종 go/no-go 판정
