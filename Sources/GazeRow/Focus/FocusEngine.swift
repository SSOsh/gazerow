...
struct FocusEngine: Equatable {
    ...
    mutating func focusNearest(
        to point: CGPoint,
        hysteresisMargin: CGFloat = 0
    ) -> FocusEngineEvent? {
        let controller = GazeFocusController(hysteresisMargin: hysteresisMargin)
        guard let nearestItem = controller.nearestItem(
            to: point,
            in: items,
            current: focusedItem
        ) else {
